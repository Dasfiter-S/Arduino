...
//PATTERN RECOGNITION=========================================================================================
void drive(int state) { //priority threading Turn -> forward -> small corrections -> big corrections
  if (state == 7) {   //00111
      gotTurn();
  }
  else if (state == 28) {   //11100
      gotTurn();
  }
  else if (state == 31) {   //11111 second time we encounter this it has to be a right turn
      gotTurn();
  }
  else if (state == 4) {    //00100
      forward();
  }
  else if (state == 0) {    //00000
    if (startTrack) {
        forward();
    }
  }
  //Correction routines------------------------------------
  else if (state == 12) {    //01100
    slightRight();
    //forward();
    //Serial.println("Correct right");
  }
  else if (state == 8) {    //01000
    slightRight();
    //Serial.println("Correct right");
  }

  else if (state == 6) {    //00110
    slightLeft();
    //forward();
    //Serial.println("Correct left");
  }

  else if(state == 2) {    //00010
    slightLeft();
    //Serial.println("Correct left");
  }
}

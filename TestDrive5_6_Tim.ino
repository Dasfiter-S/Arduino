...
bool isTurn(){
  if(path[pathState]==2){
    ...
    //if sensor[4] is reading the line, double check it and the other turn sensors.
    for(int i =0;i<2;i++){
      readSensor(2);
...
//starts the motors going in the direction of the next turn.  Based on pathState.
void mdPathState(){
...
//move forward along the line.  Includes correct left and correct right code.
void mdCorrectForward(){
  ...
  if(sensor[1]){  //left correction sensor is reading
  ...
  else if(sensor[3]){  //right correction sensor is reading
  ...
  else if(sensor[2]){  //center sensor is reading
    mdForward(1);
  }
  else{
    readSensor(0);

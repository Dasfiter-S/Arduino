...
ServoInfo servo2 = {34, 52, 1258, 92};
...
void setup() {
  ...
  rotateServos(); //unlatches the ring arm.
}
...
void loop() {
  ...
  if(isTurn()){
    ...
    lastCorrect=0;
    pathState++;  
    if(pathState>pathLength){ //done with path
...
bool isTurn(){
  if(path[pathState]==2){
    ...
    if(sensor[4]){
      ...
      for(int i =0;i<2;i++){
        readSensor(2);
        if(!sensor[2] and lastCorrect !=1) return false;
        readSensor(3);
    ...
    if(sensor[0]){
      ...
      for(int i =0;i<2;i++){
        //readSensor(2);
        if(!sensor[2] and lastCorrect != 2) return false;
        readSensor(1);
...
void rotateServos() {
  ...
  myServo2.write(servo2.turnSpeed);
